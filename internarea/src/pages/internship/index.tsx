...
const parseStipend = (stipendStr: string): number => {
  ...
  // Clean string and extract numbers (e.g. "₹15,000/month" -> 15000)
  const cleaned = stipendStr.replace(/[^0-9]/g, "");
...
const InternshipListing = () => {
  ...
  const [filter, setfilters] = useState({
    ...
    workFromHome: false,
    partTime: false,
    stipend: 0, // Default to 0 minimum stipend
  });
  ...
  useEffect(() => {
    const filtered = internshipData.filter((internship: any) => {
      ...
      const matchesWFH = !filter.workFromHome || 
        ...
        (internship.location || "").toLowerCase().includes("home") || 
        (internship.location || "").toLowerCase().includes("wfh");

      const matchesPartTime = !filter.partTime || 
        (internship.title || "").toLowerCase().includes("part-time") ||
        (internship.title || "").toLowerCase().includes("part time") ||
        (internship.aboutInternship || "").toLowerCase().includes("part-time") ||
        (internship.aboutInternship || "").toLowerCase().includes("part time");

      // Minimum stipend constraint check
      const minRequired = filter.stipend * 1000; // Stipend slider is 0-100 representing ₹0-₹100K
      const internshipStipendVal = parseStipend(internship.stipend);
      const matchesStipend = internshipStipendVal >= minRequired;

      return matchesCategory && matchesLocation && matchesWFH && matchesPartTime && matchesStipend;
    });
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    <div className="flex items-center space-x-2 text-gray-600">
                      ...
                      <div>
                        <p className="text-xs text-gray-400 font-semibold">Start Date</p>
                    ...
                    <div className="flex items-center space-x-2 text-gray-600">
                      ...
                      <div>
                        <p className="text-xs text-gray-400 font-semibold">Stipend</p>
